...
function applyLocation(loc) {
  ...
  if (!loc || loc.blank) {
    ...
    if (loc.tab === "alchemy") {
      ...
      Alchemy.applyLocation({ mode: loc.mode, constellationMode: loc.constellationMode, contextView: loc.contextView, recordId: loc.recordId, instant: true });
...
export function init() {
  ...
  const av = document.getElementById("alchemy-view");
  if (av) obs.observe(av, { attributes: true, attributeFilter: ["data-alch-mode-current", "data-const-mode-current", "data-context-view", "data-alch-detail"] });
}
