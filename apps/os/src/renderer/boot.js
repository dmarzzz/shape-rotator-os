...
async function wireAppUpdateChip() {
  ...
  if (info) {
    ...
    // Stash the platform so the manual-update branch can pick the smooth
    // unsigned-mac in-place swap vs the linux .deb open-the-file path.
    if (info.platform) chip.dataset.platform = info.platform;
  } else {
    chip.textContent = "v?";
  }
...
//   manual+mac (unsigned-mac) — click runs the in-place swap helper
//           (fg:install-mac-update): swap .app, de-quarantine, relaunch — the
//           same one-click "install & restart" feel, dmg-open as fallback.
//   manual+deb (linux .deb)  — click opens the downloaded installer to run.
let _updateReady = null;   // { mode, path, mac } while a staged update is waiting
function setUpdateReady(mode, ctx) {
  ...
  icon.classList.remove("fading");
  const mac = !!ctx?.mac;
  _updateReady = { mode, path: ctx?.path || null, mac };
  icon.dataset.state = "ready";
  // win/AppImage (restart) and unsigned-mac (in-place swap) both install +
  // relaunch on click → rotate-cw + "install & restart". Only linux .deb still
  // drops the user at the file → check + "open installer".
  const seamless = mode === "restart" || mac;
  const glyph = seamless
    ? `<path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/>` // rotate-cw
    : `<path d="M20 6 9 17l-5-5"/>`;                                                          // check
  const label = seamless ? "install & restart" : "open installer";
  icon.innerHTML =
  ...
  icon.title = mode === "restart"
    ...
    : (mac
        ? "update downloaded — click to install & relaunch now"
...
async function onUpdateIconClick() {
  ...
  if (state === "ready" && _updateReady) {
    try {
      if (_updateReady.mode === "restart") {
        await window.api?.applyUpdateAndRestart?.();
      } else if (_updateReady.mac) {
        // unsigned-mac: in-place swap + de-quarantine + relaunch; fall back to
        // opening the dmg for the manual drag if the swap can't run.
        const r = await window.api?.installMacUpdate?.(_updateReady.path);
        if (!r?.ok) await window.api?.openDownloadedInstaller?.(_updateReady.path);
      } else {
        // linux .deb: open the downloaded installer (no sudo from here).
        await window.api?.openDownloadedInstaller?.(_updateReady.path);
      }
    } catch {}
    return;
  }
  ...
  try {
    ...
    if (chip?.dataset.canAutoUpdate === "1") {
      ...
      // unsigned-mac / .deb: stream the asset to ~/Downloads with progress.
      // mac lands on the smooth in-place "install & restart"; .deb on the
      // "open installer" file step.
      const res = await window.api?.downloadAndRevealUpdate?.();
      if (res?.ok) setUpdateReady("manual", { path: res.path, mac: chip?.dataset.platform === "darwin" });
      else setUpdateIcon("available");
    }
  } catch {
    setUpdateIcon("available");
