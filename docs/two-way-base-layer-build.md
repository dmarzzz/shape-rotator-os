...
## Information-pipeline pass

A second audit (reader contracts / overlay chain / source→surface coverage / build+web) drove a pipeline-wide pass — the deferred sibling-reader work, done properly:

- **One shared read primitive:** `fetchAnon(path, {bearer, accept})` → `{rows, source, error}` (granular `supabase`|`unconfigured`|`error`, exact error strings preserved). `getAnonRows` is now a thin wrapper. **6 of 7** hand-rolled readers route through it — `calendar`, `standing`, `sphere`, `releases` (+ a new test, it was uncovered), `distillations`, and `evidence` ×3 (cohort-bearer readers pass the JWT + relabel `supabase-cohort`). Only `supabase-articles` stays hand-rolled (bespoke config + dual app→public fallback + 3rd bearer — partial fit).
- **Broke a structural cycle:** extracted the connection config (`readSupabaseConfig` + keys) from `supabase-evidence` into a leaf `supabase-config.mjs` (re-exported for back-compat). `supabase-anon-write` now depends on that leaf, not the big evidence module — so `supabase-evidence` can use `fetchAnon` without a cycle, and the dep graph is cleaner.
- **Pipeline bug fixed:** `cohort-source` `normalize()` now strips `collab-edge:` alongside the other derived edges, so a persisted LS snapshot no longer leaks a stale collaboration edge into `declaredDeps` permanently.
- **Audit confirmed clean** (no action needed): no duplicate fetches across the overlay chain; `signatureOf()` covers every overlay's surface key (incl. `cohort_events`); the activity route is wired end-to-end; OS↔web surface is one build + a public-redaction transform (not drifted); dual-source surfaces (calendar/standing/releases) are all clean live-first/bundle-fallback.
- **Deferred (flagged for follow-up):** `supabase-articles` onto `fetchAnon` (needs a full-URL guard); the `buildWhatsNew` (build) vs `buildWhatsNewFeed` (runtime fallback) divergence — they read *different* inputs (artifacts vs bundled surface), so they can't trivially share, but the runtime copy should at least align its window-filter + commit-count source; the `public_cohort_insight_cards` anon view is published-but-unread (decide: add a public reader so the web build shows collaboration edges, or drop the anon grant); the `cohort_skill_vocab` "anon-readable" migration comment is misleading (it's a server-side validator reference, no anon GRANT).
